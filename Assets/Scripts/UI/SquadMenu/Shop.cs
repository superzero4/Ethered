...
namespace SquadSystem
{
    ...
    {
        [Header("Squad Members Shop Settings")] [SerializeField]
        private int minSquadMembersInShop = 1;

        [SerializeField] private int maxSquadMembersInShop = 3;
        [SerializeField] private EncounterInfo squadMembersList;

        [SerializeField]
        private GameObject squadMemberShopContainer; // The container where the squad members will be displayed (UI)

        [SerializeField]
        private GameObject squadMemberShopPrefab; // The prefab of the squad member button in the shop (UI)

        [Header("Global Upgrades Shop Settings")] [SerializeField]
        private int minGlobalUpgradesInShop = 1;

        [SerializeField] private int maxGlobalUpgradesInShop = 3;
        [SerializeField] private List<Upgrade> globalUpgradesList;

        [SerializeField] private GameObject
            globalUpgradesShopContainer; // The container where the global upgrades will be displayed (UI)

        [SerializeField]
        private GameObject globalUpgradesShopPrefab; // The prefab of the global upgrade button in the shop (UI)

        [Header("Items Shop Settings")] [SerializeField]
        private int minItemsInShop = 1;
        ...
        {
            for (int i = 0; i < Mathf.Max(squadMembersInShop, squadMembersList.Units.Units.Count); i++)
            {
                var squadMemberPicked = squadMembersList.Units.Units[i];
                ...
                squadMemberShopUI.SetParameters(
                    squadMemberPicked.VisualInformations.Name + "\n" + squadMemberPicked.ArchetypeName,
                    squadMemberPicked.MaxHealth,
                    squadMemberPicked.Armor,
                    100,
                    100
                );
        ...
        public void GenerateGlobalUpgradesShopWithTheList(int globalUpgradesInShop)
        {
            for (int i = 0; i < globalUpgradesInShop; i++)
            {
                ...
                globalUpgradesList.RemoveAt(
                    globalUpgradePicked); // Remove the global upgrade from the list to avoid duplicates
            }
        }
